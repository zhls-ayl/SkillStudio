# SkillsMaster Development Guide
...
│   └── SkillsMaster/
│       ├── App/
│       │   └── SkillsMasterApp.swift    # 应用入口（≈ main()）
...
2. **选择目标**: 顶部工具栏选择 `SkillsMaster` scheme 和 `My Mac`
3. **运行**: 点击 ▶ 按钮 或 `Cmd + R`
4. **停止**: 点击 ■ 按钮 或 `Cmd + .`
...
let name = "SkillsMaster"          // 类型推断为 String
...
```swift
import XCTest
@testable import SkillsMaster  // @testable 让测试可以访问 internal 成员
...
```bash
# 1. 创建 bundle 结构
mkdir -p SkillsMaster.app/Contents/MacOS
mkdir -p SkillsMaster.app/Contents/Resources

# 2. 复制可执行文件
cp .build/release/SkillsMaster SkillsMaster.app/Contents/MacOS/

# 3. 创建 Info.plist
cat > SkillsMaster.app/Contents/Info.plist << 'EOF'
...
<dict>
    ...
    <string>com.skillsmaster.app</string>
...
codesign --deep --force --sign "Developer ID Application: Your Name" SkillsMaster.app

# 3. 创建 DMG
hdiutil create -volname SkillsMaster -srcfolder SkillsMaster.app -ov SkillsMaster.dmg

# 4. 提交 notarize
xcrun notarytool submit SkillsMaster.dmg --apple-id your@email.com --team-id XXXX --password @keychain:notarize

# 5. 等待并 staple
xcrun stapler staple SkillsMaster.dmg
```
...
cask "skillsmaster" do
  version "0.1.0"
  sha256 "xxx"
  url "https://github.com/xxx/SkillsMaster/releases/download/v#{version}/SkillsMaster.dmg"
  name "SkillsMaster"
  homepage "https://github.com/xxx/SkillsMaster"
...
Package.swift 的 `Sources/SkillsMaster` 目录下的所有 `.swift` 文件会自动包含。确保文件放在正确的路径下。
