...
final class SkillManager {
    ...
    /// Loaded from `~/.agents/.skillstudio-repos.json` on first refresh.
    ...
    /// F12: SkillStudio private commit hash cache, independent of .skill-lock.json
    /// Stored in ~/.agents/.skillstudio-cache.json, doesn't pollute npx skills' lock file format
    ...
    ///
    /// Link info is stored in SkillStudio private cache (~/.agents/.skillstudio-cache.json),
    ...
    /// Check if SkillStudio app itself has a new version
    ///
    /// Flow:
